...
// EFFECT STUFF GOES HERE
		if (target.getTemplateId() == AlchItems.potionIdHeal) {
			if (!healCooldown.containsKey(performer.getWurmId()) || healCooldown.get(performer.getWurmId()) + 300000 < System.currentTimeMillis()) {
				...
				Items.destroyItem(target.getWurmId());
				healCooldown.put(performer.getWurmId(), (long) (System.currentTimeMillis()+(30000+( (target.getCurrentQualityLevel()/10) * 21000 ))));
				healToxicity.put(performer.getWurmId(),0);


			} else if (healToxicity.get(performer.getWurmId()) < 1&& healCooldown.get(performer.getWurmId()) >System.currentTimeMillis()) {
				performer.getCommunicator().sendAlertServerMessage(
						"You should wait another "+
						(((healCooldown.get(performer.getWurmId())-System.currentTimeMillis())/1000)+1)+
						"Seconds before trying to imbibe a potion again");

				healToxicity.put(performer.getWurmId(),1);

			} else if (healToxicity.get(performer.getWurmId()) > 0 && healCooldown.get(performer.getWurmId()) >System.currentTimeMillis()) {
